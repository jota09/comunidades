...
public class RegistroDAO implements GestionDAO {
    ...
    public Object getObject(Object object) {
        ...
        try {
            ...
            String sql = "Select codigo,comunidad_codigo,fecha_vencimiento,correo from registro where codigo_generado=? ";
            PreparedStatement pS = con.prepareStatement(sql);
            pS.setString(1, registro.getCodigoGenerado());
            ResultSet rS = pS.executeQuery();
            if (rS.next()) {
                registro.setCodigo(rS.getInt(1));
                registro.setFechaVencimiento(rS.getTimestamp(3));
                Comunidad comunidad = new Comunidad();
                comunidad.setCodigo(rS.getInt(2));
                registro.setComunidad(comunidad);
                registro.setCorreo(rS.getString(4));
            }
            rS.close();
            pS.close();
    ...
    public int insertObject(Object object) {
        ...
        try {
            con = ConexionBD.obtenerConexion();
            String sql = "insert into registro(codigo_generado,comunidad_codigo,correo,fecha_vencimiento) values(?,?,?,?)";
            PreparedStatement pS = con.prepareStatement(sql);
            pS.setString(1, registro.getCodigoGenerado());
            pS.setInt(2, registro.getComunidad().getCodigo());
            pS.setString(3, registro.getCorreo());
            pS.setTimestamp(4, registro.getFechaVencimiento());
