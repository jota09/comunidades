...
function eliminaImagen(padre, name) {
    ...
    //console.log("nombreEliminar:"+name);
    delete array[name];
    //console.log("array:"+array);
    tamArray--;
    //console.log("tamaÃ±o:"+tamArray);
}
...
function inicioClasificado() {
    ...
    $.get("ClasificadoControlador", {opc: 10}, function (data) {
        tipoArt = data;
        mostrarArticulo('0,' + $("#paginaClasificado").val(), '1', 1);
...
function mostrarArticulo(paginas, opcion, tipoOpcion)
{
  ...
  var parametros={rango:((paginas!=undefined)?paginas:""),tipo:tipoOpcion,opc:15,cat:categ,buscar:search};
        ...
        async: true})
            .done(function (data) {
                ...
                {   
                    ...
                    fila += "<td>";
                    fila += "<button class=\"btn btn-info aprobar\" onclick=\"aprobarArt('16','" + array[i].codigo + "','" + array[i].titulo + "') \"><span class=\"glyphicon glyphicon-ok\" title=\"aprobar clasificado\"></span></button>";
                    fila += "<button class=\"btn btn-warning devolver\"  onclick=\"devolverClasificado(" + array[i].codigo + ",'" + array[i].titulo + "') \"><span class=\"glyphicon glyphicon-repeat\" title=\"devolver clasificado\"></span></button>";
                    ...
                    fila += "<button class=\"btn btn-danger borrar\"  onclick=\"borrarClasificado(" + array[i].codigo + ",'" + array[i].titulo + "') \"><span class=\"glyphicon glyphicon-remove\" title=\"borrar clasificado\"></span></button>";
                    fila += "</tr>";
...
{
    mostrarArticulo('0,' + $("#paginaClasificado").val(), 'undefined', 20);
}
function devolverClasificado(codArt, titulo) {
    bootbox.confirm({
        title: "Devolver Clasificado",
        message: "¿Esta seguro que desea devolver el clasificado con título <strong>" + titulo + "</strong> ?",
        buttons: {
            cancel: {
                label: '<span class="glyphicon glyphicon-remove-sign"></span> Cancelar',
                className: 'btn-danger'
        ...
        callback: function (result) {
            if (result) {
                bootbox.prompt({
                    title: "Ingrese la observaci&oacute;n asociada a la devoluci&oacute;n del clasificado",
                    inputType: 'textarea',
                    buttons: {
                        cancel: {
                            label: '<span class="glyphicon glyphicon-remove-sign"></span> Cancelar',
                        ...
                        confirm: {
                            label: '<span class="glyphicon glyphicon-ok-sign"></span> Enviar',
                            className: 'btn-success'
                        }
                    },
                    callback: function (result) {
                        if (result != null) {
                            $.ajax({
                                url: 'ClasificadoControlador',
                                data: {opc: 17, cod: codArt, obs: result, tit: titulo},
                                type: 'GET',
                                async: true,
                                beforeSend: function(){
                                ...
                                $('.borrar').attr("disabled", true);}
                                        ...
                                        redirigir('administrarClasificados.html');
                                    })
                                    .fail(function (jqXHR, textStatus, errorThrown) {
                                        bootbox.alert({
                                            message: "Error al devolver el clasificado, comuniquese con el administrador...",
                                            className: 'bb-alternate-modal'
                                        });
                                    });
                        }
...
function aprobarArt(opc, codArt, titulo) {
    bootbox.confirm({
        title: "Devolver Clasificado",
        message: "¿Esta seguro que desea aprobar el clasificado con título <strong>" + titulo + "</strong> ?",
        buttons: {
            cancel: {
                label: '<span class="glyphicon glyphicon-remove-sign"></span> Cancelar',
                className: 'btn-danger'
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    ...
                    if (datos == 1) {
                        bootbox.alert("Ha sido aprobado el clasificado", function () {
                            $('.aprobar').attr("disabled", false);
                            $('.devolver').attr("disabled", false);
                            $('.borrar').attr("disabled", false);
                            redirigir('administrarClasificados.html');
                        ...
                        .fail(function (jqXHR, textStatus, errorThrown) {
                            bootbox.alert({
                                message: "Error al aprobar el clasificado, comuniquese con el administrador...",
                                className: 'bb-alternate-modal'
                            });
                        });
            }
