...
<%  ArticuloFachada artFachada = new ArticuloFachada();
    ...
    List<String> paginas = utilitarias.Utilitaria.getPaginacion(numRegistrosTabla, 10);
%>
    ...
    <div class="row">
        <div class="col-xs-7 col-sm-6 col-md-3 col-lg-2">
            <h2>
                ...
                <%= LecturaConfig.getValue("titulo", "/configComunidades/noticias.properties")%>
            </h2>
        </div>
        ...
        <div role="tabpanel" class="tab-pane active" id="todos">
            ...
            <div class="row">
                <button class="btn btn-primary" id="btn1">
                    <%= LecturaConfig.getValue("nombreBoton2", "/configComunidades/noticias.properties")%>
                </button>
                <button class="btn btn-primary" id="btn2">
                    <%= LecturaConfig.getValue("nombreBoton3", "/configComunidades/noticias.properties")%>
                </button>
                <button class="btn btn-primary" id="btn3">
                    <%= LecturaConfig.getValue("nombreBoton4", "/configComunidades/noticias.properties")%>
                </button>
                <button class="btn btn-primary " id="btn4">
                    <%= LecturaConfig.getValue("nombreBoton5", "/configComunidades/noticias.properties")%>
                </button>
            </div>
            <br/>           
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    <li id="<%= pag + "-" + cont%>" style="cursor:pointer;" >
                    ...
                    %>
                    <li id="<%=cont%>" style="cursor:pointer;"><a  onclick="mostrarNoticia('<%=p%>', '<%=cont%>')" ><%= cont%></a></li>
                        <%
                                cont++;
                            }
                        %>                  
            ...
            <button class="btn btn-primary" id="btn5">
                <%= LecturaConfig.getValue("nombreBoton6", "/configComunidades/noticias.properties")%>
            </button>
            <button class="btn btn-primary" id="btn6">
                <%= LecturaConfig.getValue("nombreBoton7", "/configComunidades/noticias.properties")%>
            </button>
        </div>        
